 ...
 */
function dctx_categorized_blog() {

	$category_count = get_transient( 'dctx_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'dctx_categories', $category_count );
	}
 ...
 */
function dctx_get_attachment_id_from_url( $attachment_url = '' ) {
 ...
 * @return string
 */
function dctx_get_placeholder_unsplash( $args = array() ) {
	...
	if ( ! is_numeric( $args['width'] ) ) {
		throw new Exception( esc_html__( 'Width must be an integer', 'dctx' ) );
	}

	if ( ! is_numeric( $args['height'] ) ) {
		throw new Exception( esc_html__( 'Height must be an integer', 'dctx' ) );
	}
	...
	$alt = apply_filters( 'dctx_placeholder_image_alt', esc_html__( 'WebDevStudios Placeholder Image', 'dctx' ) );
 ...
 * @return string
 */
function dctx_copyright_year( $atts ) {
...
add_shortcode( 'dctx_copyright_year', 'dctx_copyright_year', 15 );
...
if ( defined( 'WPSEO_VERSION' ) ) {
	 ...
	 * @since  NEXT
	 */
	function dctx_move_yoast_to_bottom() {
		return 'low';
	}
	add_filter( 'wpseo_metabox_prio', 'dctx_move_yoast_to_bottom' );
}
 ...
 * @author jomurgel
 */
function dctx_get_the_content( $content ) {
...
add_filter( 'the_content', 'dctx_get_the_content', 20 );
 ...
 */
function dctx_get_theme_colors_gutenberg() {
